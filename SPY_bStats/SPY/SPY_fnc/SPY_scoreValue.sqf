...
private ["_type", "_condition", "_multiplier", "_score", "_base"];
	...
	case "kill": {
		...
		switch (_condition) do {
			...
			case 1: {
				...
				if ((_multiplier <= 50)) then {_score = (_base + (round ((50 - _multiplier) / 5)));} else {
			
					if ((_multiplier > 400)) then {_score = (_base + (round (1 + ((_multiplier - 400) / 100))));} else {_score = _base;};
			...
			case 2: {
				...
				if ((_multiplier <= 100)) then {_score = (_base + (round ((100 - _multiplier) / 5)));} else {
			
					if ((_multiplier > 1000)) then {_score = (_base + (round (1 + ((_multiplier - 1000) / 100))));} else {_score = _base;};
