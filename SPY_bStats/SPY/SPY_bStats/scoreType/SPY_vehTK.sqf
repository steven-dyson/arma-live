...
[(format ["bstats_vehtk (%1, %2, %3, %4, %5, %6, %7, %8)", (str (typeOf _vehicle)), _killerUID, time, (str _weapon), _score, _distance, (str str _killerPos), (str str _vehiclePos)])] call uplink_exec;
...
if ((!(SPY_bStats_valhalla))) then {
	...
	if ((SPY_bStats_msgsEnabled)) then {
		...
		_null = [[_killerName, ([_vehicle] call SPY_displayName), ([_weapon] call SPY_displayName)], "SPY_GAMELOGIC globalChat format ['%1 DESTROYED A FRIENDLY %2 (%3)', _this select 0, _this select 1, _this select 2];", "CLIENT"] spawn JDAM_mpCB;
...
// VALHALLA MONEY AWARD
	...
	_award = (- ((_vehicle getVariable "kill_award") * 2));
		
	DAO_WF_Logic setVariable [_fundsName, ((DAO_WF_Logic getVariable _fundsName) + _award), true];
	
	if ((SPY_bStats_msgsEnabled)) then {
	
		_null = [[_killerName, ([_vehicle] call SPY_displayName), _award], "SPY_GAMELOGIC globalChat format ['%1 DESTROYED A FRIENDLY %2 ($%3)', (_this select 0), (_this select 1), (_this select 2)];", "CLIENT"] spawn JDAM_mpCB;
